...
#ifndef _INCLUDE_REQNUMBERRULESXML_H_
#define _INCLUDE_REQNUMBERRULESXML_H_
...
/*
 * A subclass of ReqNumberRules that constructs itself based on an XML node,
...
{
public:
    virtual ~ReqNumberRulesXML(void);
    virtual const char *outputConstructor(void);
    static LexerRules *buildRules(XMLNode *node);

private:
        // Use buildRules(), which calls this.
    ReqNumberRulesXML(void);
}; // class ReqNumberRulesXML

#endif // !defined _INCLUDE_REQNUMBERRULESXML_H_

// end of ReqNumberRulesXML.h ...
